...
(function($) {
    ...
    $.fn.imageupload = function(methodOrOptions) {
        ...
        return this.filter('div').each(function() {
            if (methods[methodOrOptions]) {
                ...
                throw new Error('Método "' + methodOrOptions + '" no está definido en imageupload.');
            }
        });
    };
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'Archivo muy grande (Máximo ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Formato de Imagen Valida.');
        }
        else {
            callback(false, 'Formato de Imagen Invalido.');
        }
    }
    ...
    function isValidImageUrl(url, callback) {
        ...
        image.onload = function() {
            if (!timeout) {
                ...
                if ($.inArray(fileExtension, options.allowedFormats) > -1) {
                    callback(true, 'URL de Imagen Valida.');
                }
                else {
                    callback(false, 'URL de Imagen Invalida.');
                }
            }
        };
        ...
        image.onerror = function() {
            if (!timeout) {
                window.clearTimeout(timer);
                callback(false, 'La imagen no se pudo encontrar.');
            }
        };
        ...
        timer = window.setTimeout(function() {
            ...
            callback(false, 'Tiempo de carga de imagen excedido.');
        }, timeoutMs);
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('Seleccionar');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').text('Seleccionar');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onerror = function() {
                    $fileTab.prepend(getAlertHtml('Error cargando el archivo de imagen.'));
                    $fileInput.val('');
                };
    ...
    function submitImageUrl($urlTab) {
        ...
        if (!url) {
            $urlTab.prepend(getAlertHtml('Por favor ingrese la URL de la imagen.'));
            return;
        }
